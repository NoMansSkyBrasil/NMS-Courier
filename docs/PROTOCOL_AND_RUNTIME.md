...
Status: version 1 control-plane schemas and TypeScript fixtures are implemented in `packages/protocol`, including a bounded local item-delivery intent. The direct Python host remains a diagnostic path. An exact-build native XInput proxy has observed the live update callback, delivered Carbon ×500 and three currencies on a disposable save, and opened a free freighter offer. These were one-shot local experiments; the native proxy does not yet implement the authenticated product command channel or a general capability registry.
...
The native startup route has progressed beyond the original ASI/WinHTTP proposal: our XInput proxy loaded for the exact Steam build 179666, forwarded its imports, observed repeated `cGcApplication.Update` callbacks, and dispatched the verified local test operations. Keep the common game-callback and transport boundary, but replace the current process-specific named-event test triggers with the authenticated, bounded command queue before the Electron UI can submit delivery. Define separate capability IDs and postconditions for item grant, currency grant, inventory expansion, technology unlock, ship/multitool/freighter offer, and later multiplayer transfer. Publish only capabilities proved on the current executable fingerprint and loaded context. The UI may display future action groups but must keep unsupported actions disabled.

The earlier Ultimate ASI Loader/WinHTTP candidate was not adopted: its default initialization crashed an isolated fixture, and its loader was removed before game launch. The current Windows x64 native prototype is our own `XINPUT9_1_0.dll` proxy loaded during game startup. It is an exact-build research installation and must not overwrite an unexpected DLL. The private Python bridge remains diagnostic evidence, not the current live delivery path.

For the exact local Steam 179666 executable fingerprint, the bridge checks the in-memory prologue at `cGcApplication.Update` RVA `0x2D7500` and has observed repeated live callbacks. The exact-build `cGcInventoryStore.Add` target at RVA `0x4CE130` delivered Carbon ×500 through that callback; the user confirmed the item and save persistence. These observations apply only to the pinned executable and tested context. No engine call may run from a pipe thread. Installation must detect an existing `xinput9_1_0.dll`, never overwrite an unexpected file, and provide exact-file rollback without touching the game executable.

The planned desktop-owned private per-session Named Pipe is not yet connected to the native DLL. Preserve protocol versioning, current-user access controls, peer PID/session checks, ephemeral authentication, bounded frames, and explicit operation IDs. The I/O worker may parse and validate frames, but it only enqueues a bounded immutable command. The verified game callback rechecks build, save/player readiness, deadline, and operation state immediately before one native call. A response lost after dispatch is `unknown`; reconnection must not automatically replay the mutation. The live Carbon, currency, and freighter probes used narrow process-specific events and do not grant general delivery capability merely because the DLL loads.
