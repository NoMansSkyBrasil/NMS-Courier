...
const semanticVersionSchema = z.string().regex(/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/)
...
export const capabilitySchema = z.object({
  ...
  implementationLevel: z.enum(['not_implemented', 'unknown', 'experimental', 'verified']),
  available: z.boolean(),
...
export const controlCommandSchema = z.discriminatedUnion('action', [
  z.object({ action: z.literal('status.snapshot'), payload: z.object({}).strict() }).strict(),
  z.object({ action: z.literal('capabilities.query'), payload: z.object({}).strict() }).strict(),
  z.object({ action: z.literal('operation.status'), payload: z.object({ operationId: opaqueIdSchema }).strict() }).strict(),
  z.object({ action: z.literal('operation.cancel'), payload: z.object({ operationId: opaqueIdSchema }).strict() }).strict()
])
