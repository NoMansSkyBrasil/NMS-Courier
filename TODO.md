...
- [~] The independently authored [freighter-specific reward experiment](runtime/mods/freighter_specific_reward/README.md) opened a free offer in ordinary gameplay, and the user acquired the freighter without a Units debit. The generated freighter was C class with 35 main slots and a 21-position technology grid, of which 13 slots were valid, despite S class and layout 120 in the reward data. Identify a verified per-offer generation or native path for S class and 120/60 unlocked slots; do not repeat the one-shot event in the same process.
